  ...
  methods: {
    handleScroll() {
      if (window.pageYOffset > 20) {
        if (this.$refs.backTop.classList.contains("hide")) {
          this.$refs.backTop.classList.toggle("hide");
        }
      } else {
        this.$refs.backTop.classList.add("hide");
      }
    },
    scrollToTop() {
      $("html, body").animate(
...
<template>
  <div class="back-top hide" ref="backTop" title="回頂部" @click="scrollToTop">
  </div>
</template>
